...
struct TurtleSceneView: View {
    ...
    private var daylight: CGFloat { CGFloat(TimeOfDay.daylight()) }
    private var isNighttime: Bool { TimeOfDay.isNight() }
            ...
            case .active(let location):
                ...
                if !s.isWalking && !s.isEating && !s.isSleeping && !isProcessing && !needsAttention {
                    ...
                    if abs(dist) > 0.05 {
                        ...
                        // Clamp to edges
                        s.walkX = max(-0.41, min(0.45, s.walkX))
                    }
                }
            case .ended:
